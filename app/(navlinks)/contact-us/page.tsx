...
export default function ContactPage() {
  ...
  const handleWhatsAppRedirect = () => {
    const whatsappNumber = '919873750969'
    const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`
    window.open(whatsappUrl, '_blank')
  }

  const handlePhoneClick = () => {
    window.location.href = 'tel:+919873750969'
  }

  const handleAddressClick = () => {
    window.open(
      'https://maps.google.com/?q=NASSCOM+CoE+IoT+Hartron+Innovation+Campus+Plot+1+Udyog+Vihar+Phase+1+Sector+20+Gurugram+Haryana+122022',
      '_blank'
    )
  }
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold mb-4">
            <span className="text-[#318832]">
              {content.contactUs.title.part1}
            </span>{' '}
            <span className="text-[#DD2E29]">
              {content.contactUs.title.part2}
            </span>
          </h1>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6">
              <Card className="transform transition-all duration-300 hover:scale-[1.02]">
                <CardContent className="flex items-center gap-6 p-6 cursor-pointer">
          ...
          <div className="lg:mt-16 mt-8">
            <Card className="p-8">
              <h2 className="text-2xl font-semibold mb-8 text-center">
                {content.contactUs.sendMessage}
              </h2>
              <div className="space-y-6">
                <Textarea
                  value={message}
                  onChange={e => setMessage(e.target.value)}
                  placeholder={content.contactUs.messagePlaceholder}
                  className="min-h-[200px] resize-none p-4"
                />
                <Button
                  onClick={handleWhatsAppRedirect}
                  className="w-full bg-green-600 hover:bg-green-700 text-white py-6 text-lg font-medium transition-all duration-300"
                >
                  <svg
                  ...
                  {content.contactUs.whatsappPlaceholder}
                </Button>
              </div>
            </Card>
          </div>
